...
def save_answer(user_answer_id):
    question_id = session["quiz_question_id"]
    user_id = session["user_id"]

    sql = "INSERT INTO user_answers (user_id, question_id, answer_id) " \
        "VALUES(:user_id, :question_id, :answer_id)"
    db.session.execute(sql, {"user_id":user_id, "question_id":question_id, "answer_id":user_answer_id })
    db.session.commit()
